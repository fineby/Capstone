...
def request1():
    ...
    while w:
        ...
        if z.isdigit() and y.isdigit() and m.isdigit():
            if len(z)<6 and len(y)==4 and len(m)<3:
                ...
                if (y+m in year_month) and (z in zip):
                    ...
                    orderBy(F.col('TIMED').desc()).show(1000)
                else: print('\nEntered data does not mutch database records') 
            else: print('\nEntered data is incorrect')                 
        else: print('\nIncorrect input. Try again.')
        i_1=input('Do you want to make same another request (enter 1 for Yes, any other input for Main menu)?: ')
        if i_1=='1': continue
        else: 
            w = False
            os.system('cls')
        ...
        if z.isdigit():
            if int(z) in list(range(1, len(df_trans_types)+1)):
                ...
                t_type.select('TRANSACTION_TYPE', F.col("count").alias("Total number for a given type"), \
                 F.format_number('sum(TRANSACTION_VALUE)', 2).alias("Total values of transactions for a given type")).show()
            else: print('\nIncorrect input, try again')
...
def request3():
    ...
    while w:
        ...
        z=input("\nEnter number from the list of states: ")
        if z.isdigit():
            if int(z) in list(range(1, len(df_branch_state)+1)):
                ...
                F.format_number('sum(TRANSACTION_VALUE)', 2).alias(f"Total values of transactions for {df_branch_state[int(z)-1]}")).show()
            else: print('\nIncorrect input, try again')                
        else: print('\nOnly numbers allowed, try again')
        i_1=input('Do you want to make same another request (enter 1 for Yes, any other input for Main menu)?: ')
        if i_1=='1': continue
...
def request5():
    ...
    while w:
        ...
        ssn_inp=input("Enter SSN number : ")
        if ssn_inp.isdigit():
            if (int(ssn_inp) in df_cust.select('SSN').rdd.map(lambda x: x[0]).collect()):
                count=1
                menu_choises={}
                for i in df_cust.columns[1:-1]:
                    ...
                    count+=1  
                z=input("\nEnter number for the field to modify: ")
                if z.isdigit():
                    z=int(z)
                    if z<count and z in menu_choises.keys():
                        ...
                        elif z==4 and f_modif.isdigit() and len(f_modif)==16: f_modif_ins=int(f_modif)
                        ...
                        elif z==5 or z==6 or z==7 or z==8: pass
                        else:
                            print('\nIncorrect input. Try again.')
                            time.sleep(3)
                            continue
...
def request6():
    ...
    while w:
        ...
        m=input("Enter Month: ")
        if z.isdigit() and y.isdigit() and m.isdigit():
            if len(z)<17 and len(y)==4 and len(m)<3:
                if len(m) !=2: m='0'+m
                z=int(z)
                res=spark.sql(f"SELECT CUST_CC_NO, ROUND(SUM(TRANSACTION_VALUE),2) AS MONTLY_BILL FROM credit_table \
        ...
        if date1.isdigit() and date2.isdigit():
            if int(date1)==int(date2)==0:
                ...
                try:
                    ...
                    time.sleep(3)
                    continue
            print('\nCustomer could be identificated by SSN or personal data (First Name, Middle name, Last name)')
            pd_inp=input('For identification by personal data enter 1 or any other input for SSN identification: ')
            if pd_inp!='1':
                ssn_inp=input("Enter SSN number : ")
                ...
                if res.rdd.isEmpty():
                    ...
                    res.withColumn("TIMED", F.date_format(F.col("TIMED"), "yyyyMMdd")).show(1000)
            else:
                cust_f=input("Enter customer First Name : ")
                cust_m=input("Enter customer Middle Name: ")
                cust_l=input("Enter customer Last Name: ")
